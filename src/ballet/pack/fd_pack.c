    ...
    for( ulong i=0UL; i<8UL; i++ ) {
      ...
      switch( root_idx & FD_ORD_TXN_ROOT_TAG_MASK ) {
        ...
        case FD_ORD_TXN_ROOT_PENALTY( 0 ): {
          fd_acct_addr_t const * accts   = fd_txn_get_acct_addrs( TXN( sample->txn ), sample->txn->payload );
          fd_acct_addr_t const * alt_adj = sample->txn_e->alt_accts - fd_txn_account_cnt( TXN( sample->txn ), FD_TXN_ACCT_CAT_IMM );
